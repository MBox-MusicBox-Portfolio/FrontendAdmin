      ...
      <div>
        ...
        <ul class="list-group ml-2 mr-2">
          <li v-for="item in filteredUsers" :key="item.id" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center mb-2">
            ...
            <div>
              <button type="button" class="btn btn-primary" @click.prevent="editUser(item)">Edit</button>
            </div>
          </li>
        </ul>
      </div>
      <editVue :userName="selectedUserName" :photo="photo" :currentRoleName="currentRole.name"  :userId="selectedUserId"  ref="editVueRef"></editVue>
    </div>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      editUser(item) {
        ...
        this.$refs.editVueRef.OpenModal(this.selectedUserId);
      },
    },
    components: {
      editVue,
